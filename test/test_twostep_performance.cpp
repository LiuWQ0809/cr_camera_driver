...
class VICTwoStepTester {
private:
    VPIStream stream_;
    VPIImage input_image_;
    VPIImage intermediate_image_;
    VPIImage output_image_;
    int width_, height_;
    bool initialized_;
    
public:
    VICTwoStepTester() : stream_(nullptr), input_image_(nullptr), intermediate_image_(nullptr),
                        output_image_(nullptr), width_(0), height_(0), initialized_(false) {}
    ...
    bool initialize(int width, int height) {
        ...
        // Create intermediate image (NV12 format)
        status = vpiImageCreate(width_, height_, VPI_IMAGE_FORMAT_NV12, 0, &intermediate_image_);
        if (status != VPI_SUCCESS) {
            std::cout << "❌ Failed to create intermediate image: " << vpiStatusGetName(status) << std::endl;
            return false;
        }
        ...
        initialized_ = true;
        std::cout << "✅ Two-step tester initialized for " << width_ << "x" << height_ << std::endl;
        return true;
    }
    ...
    void testConversionPerformance(const uint8_t* uyvy_data, uint8_t* rgb_data, int iterations = 10) {
        ...
        std::vector<double> vic1_times;
        std::vector<double> vic2_times;
        std::vector<double> cpu_times;
        std::vector<double> copy_in_times;
        std::vector<double> copy_out_times;
        
        std::cout << "\n🚀 Running two-step performance test (" << iterations << " iterations)..." << std::endl;
        std::cout << "📊 Testing UYVY→NV12→RGBA8→RGB conversion pipeline" << std::endl;
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            // Step 2: VIC hardware conversion UYVY→NV12
            auto start_vic1 = std::chrono::high_resolution_clock::now();
            status = vpiSubmitConvertImageFormat(stream_, VPI_BACKEND_VIC, input_image_, intermediate_image_, nullptr);
            if (status != VPI_SUCCESS) continue;
            
            status = vpiStreamSync(stream_);
            if (status != VPI_SUCCESS) continue;
            auto end_vic1 = std::chrono::high_resolution_clock::now();
            
            // Step 3: VIC hardware conversion NV12→RGBA8
            auto start_vic2 = std::chrono::high_resolution_clock::now();
            status = vpiSubmitConvertImageFormat(stream_, VPI_BACKEND_VIC, intermediate_image_, output_image_, nullptr);
            if (status != VPI_SUCCESS) continue;
            
            status = vpiStreamSync(stream_);
            if (status != VPI_SUCCESS) continue;
            auto end_vic2 = std::chrono::high_resolution_clock::now();
            
            // Step 4: Copy RGBA data
            ...
            // Step 5: CPU conversion RGBA→RGB
            ...
            double vic1_time = std::chrono::duration<double, std::milli>(end_vic1 - start_vic1).count();
            double vic2_time = std::chrono::duration<double, std::milli>(end_vic2 - start_vic2).count();
            ...
            vic1_times.push_back(vic1_time);
            vic2_times.push_back(vic2_time);
            ...
            if (i % 5 == 0 || i == iterations - 1) {
                std::cout << "Iteration " << (i+1) << ": Total=" << total_time << "ms "
                         << "(Copy-in=" << copy_in_time << "ms, VIC1=" << vic1_time << "ms, "
                         << "VIC2=" << vic2_time << "ms, Copy-out=" << copy_out_time 
                         << "ms, CPU=" << cpu_time << "ms)" << std::endl;
            }
        }
        ...
        std::cout << "\n📈 Two-Step Performance Analysis Results:" << std::endl;
        std::cout << "========================================" << std::endl;
        ...
        std::cout << "   ├─ VIC UYVY→NV12:         " << avg(vic1_times) << " ms (" 
                  << (avg(vic1_times)/avg(total_times)*100) << "%)" << std::endl;
        std::cout << "   ├─ VIC NV12→RGBA8:        " << avg(vic2_times) << " ms (" 
                  << (avg(vic2_times)/avg(total_times)*100) << "%)" << std::endl;
        ...
        double hardware_time = avg(vic1_times) + avg(vic2_times);
        ...
        components.push_back({"VIC UYVY→NV12", avg(vic1_times)});
        components.push_back({"VIC NV12→RGBA8", avg(vic2_times)});
...
private:
    void cleanup() {
        ...
        if (intermediate_image_) vpiImageDestroy(intermediate_image_);
...
int main() {
    ...
    VICTwoStepTester tester;
    if (!tester.initialize(width, height)) {
        std::cout << "❌ Failed to initialize two-step tester" << std::endl;
        return 1;
    }
